...
class Decisional_tree:
    def __init__(self, filename):
        ...
        self.answer_data = self.get_file_contents()
        self.qt = Questions()
        # self.rq = request()
        self.step = 0
        self.score = 0
    ...
    def get_file_contents(self):
        ...
        with open(self.file_path , 'r') as file:
            for line in file:
                stock_value = line.strip()
                tmp.append(stock_value)
        return tmp
...
def main(args):
    ...
    tree = Decisional_tree(args.filename)
    ## try:
    tree.parse_conf()
    tree.get_line_loop()
    print("Score final du patient :", tree.score)
...
if (__name__ == "__main__"):
    ...
    parser.add_argument("-f", "--filename", help="file with answer data", required=True, type=str)
